...
**Update (2025)**: As of the latest version, Docker production builds (`lean` target) now compile translations **by default** (`BUILD_TRANSLATIONS=true`). Development builds still default to `false` for faster build times.

**Historical Context**: Previously, the `BUILD_TRANSLATIONS` build argument defaulted to `false` in all Docker builds to reduce build time and image size. This caused translation issues in:

- **Docker Compose** (local development): Translations were not built by default
- **Docker Hub images**: Official images did not include compiled translations unless built with `BUILD_TRANSLATIONS=true`

**PyPI installation**: The release process has always explicitly compiled translations, so they are always included in wheel/sdist packages.

**Current behavior**:
- **Production images** (`docker build` without `DEV_MODE`): Translations are enabled by default
- **Development images** (`docker-compose up`): Translations are disabled by default for faster builds
- **PyPI packages**: Translations are always included

### How to Enable Translations in Docker

#### Production Builds (Default Enabled)

As of the latest version, production Docker builds include translations by default. Simply build the image normally:

```bash
# Production build with translations (default)
docker build -t superset .

# Or specify the lean target explicitly
docker build --target lean -t superset .
```

To disable translations in production builds (not recommended):

```bash
docker build --build-arg BUILD_TRANSLATIONS=false -t superset .
```

#### Development Builds (Docker Compose)

For development builds using `docker-compose`, translations are disabled by default to speed up builds. To enable them:
...
echo "BUILD_TRANSLATIONS=true" >> docker/.env-local
...
2. **Docker images for production**:
   ```bash
   # Translations are now enabled by default in production builds
   docker build -t apache/superset:latest .
   
   # To explicitly disable (not recommended):
   # docker build --build-arg BUILD_TRANSLATIONS=false -t apache/superset:latest .
   ```
...
To quickly check if translations are properly compiled, run:

```bash
./scripts/translations/verify_translations.sh
```

This will tell you which translation files are present and which are missing.

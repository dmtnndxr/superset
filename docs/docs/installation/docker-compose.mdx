...
#### Enabling Translations for Non-English Languages

:::tip
**Update (2025)**: Production Docker images now include compiled translations by default. This section applies primarily to **development builds** using `docker-compose`.
:::

For development builds, translations are **not** compiled by default to reduce build time. If you need to use Superset in a language other than English during development, enable translation compilation:

```bash
# Option 1: Set environment variable directly
BUILD_TRANSLATIONS=true docker compose build

# Option 2: Add to docker/.env-local file
echo "BUILD_TRANSLATIONS=true" >> docker/.env-local
docker compose build
```

This will compile both backend (`.mo`) and frontend (`.json`) translation files during the build process.

**For production deployments**, simply build the Docker image normally - translations are included by default:

```bash
# Production build (translations included by default)
docker build -t apache/superset:latest .
```

For more information about translations, see the [Translations Guide](/docs/contributing/translations).

...
:::tip
By default, translations are **not** compiled during Docker builds to reduce build time and image size.
If you need to use Superset in a language other than English, you must enable translation compilation.
:::

To enable translations, set the `BUILD_TRANSLATIONS` environment variable to `true` before building:
